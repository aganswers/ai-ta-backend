...
import beam
from beam import (
    BotContext,  # To obtain task_id
    QueueDepthAutoscaler,  # RequestLatencyAutoscaler,
)
...
def ingest(context, **inputs: Dict[str | List[str], Any]):
    ...
    def run_ingest(
        ...
        if content:
            return ingester.ingest_single_web_text(
                ...
                readable_filename,
                groups=groups,
                parse_html_text_only=parse_html_text_only,
            )
    ...
    if success_fail_dict.get("failure_ingest"):
        success_fail_dict = retry_ingest(
            supabase_client,
            posthog,
            run_ingest,
            course_name,
        ...
        if isinstance(success_fail_dict, str) or success_fail_dict.get(
            "failure_ingest"
        ):
            error = str(
                success_fail_dict
                if isinstance(success_fail_dict, str)
                else success_fail_dict["failure_ingest"]
            )
            handle_ingest_failure(
                supabase_client,
                posthog,
    ...
    try:
        if base_url:
            ...
            supabase_client.table("documents_in_progress").delete().eq("url", url).eq(
                "base_url", base_url
            ).execute()
        else:
            supabase_client.table("documents_in_progress").delete().eq(
                "course_name", course_name
            ).eq("s3_path", s3_paths).eq(
                "readable_filename", readable_filename
            ).execute()
    except Exception as e:
...
def handle_ingest_failure(
    ...
    existing = (
        supabase_client.table("documents_failed")
        .select("*")
...
class Ingest:
    ...
    def bulk_ingest(
        self, course_name: str, s3_paths: Union[str, List[str]], **kwargs
    ) -> Dict[str, None | str | Dict[str, str]]:
        """
        ...
        success_status: Dict[str, None | str | Dict[str, str]] = {
            "success_ingest": None,
            "failure_ingest": None,
        }
        try:
            ...
            for s3_path in s3_paths:
                ...
                if file_extension in file_ingest_methods:
                    ...
                    try:
                        ...
                        print(
                            f"No ingest methods -- Falling back to UTF-8 INGEST... s3_path = {s3_path}"
                        )
                    except Exception as e:
                        sentry_sdk.capture_exception(e)
                        print(
                        ...
                        success_status["failure_ingest"] = {
                            "s3_path": s3_path,
                            "error": f"We don't have a ingest method for this filetype: {file_extension} (with generic type {mime_type}), for file: {s3_path}",
                        }
            ...
            err = (
                f"❌❌ Error in /ingest: `{inspect.currentframe().f_code.co_name}`: {e}\nTraceback:\n",
    ...
    def ingest_single_web_text(
        ...
        **kwargs,
        ...
        success_or_failure: Dict[str, None | str | Dict[str, str]] = {
            "success_ingest": None,
            "failure_ingest": None,
        }
        try:
            ...
            err = (
                f"❌❌ Error in (web text ingest): `{inspect.currentframe().f_code.co_name}`: {e}\nTraceback:\n",
                traceback.format_exc(),
            )  # type: ignore
            print(err)
            sentry_sdk.capture_exception(e)
    ...
    def _ingest_single_py(self, s3_path: str, course_name: str, **kwargs):
        try:
            ...
            file_path = (
                "../media/" + file_name
            )  # download from s3 to local folder for ingest

            self.s3_client.download_file(
                os.getenv("S3_BUCKET_NAME"), s3_path, file_path
            )
            ...
            metadatas: List[Dict[str, Any]] = [
                {
                    "course_name": course_name,
                    "s3_path": s3_path,
                    "readable_filename": kwargs.get(
                        "readable_filename", Path(s3_path).name[37:]
                    ),
                    "pagenumber": "",
                    "timestamp": "",
                    "url": kwargs.get("url", ""),
            ...
            err = (
                f"❌❌ Error in (Python ingest): `{inspect.currentframe().f_code.co_name}`: {e}\nTraceback:\n",
                traceback.format_exc(),
            )
            print(err)
            sentry_sdk.capture_exception(e)
    ...
    def _ingest_single_vtt(self, s3_path: str, course_name: str, **kwargs):
        ...
        try:
            with NamedTemporaryFile() as tmpfile:
                ...
                self.s3_client.download_fileobj(
                    Bucket=os.environ["S3_BUCKET_NAME"], Key=s3_path, Fileobj=tmpfile
                )
            ...
            err = (
                f"❌❌ Error in (VTT ingest): `{inspect.currentframe().f_code.co_name}`: {e}\nTraceback:\n",
                traceback.format_exc(),
            )
            print(err)
            sentry_sdk.capture_exception(e)
    ...
    def _ingest_html(self, s3_path: str, course_name: str, **kwargs) -> str:
        ...
        try:
            response = self.s3_client.get_object(
                Bucket=os.environ["S3_BUCKET_NAME"], Key=s3_path
            )
            raw_html = response["Body"].read().decode("utf-8", errors="ignore")

            soup = BeautifulSoup(raw_html, "html.parser")

            # Optionally strip non-visible content and extract only visible text
            parse_text_only = bool(kwargs.get("parse_html_text_only", False))
            if parse_text_only:
                # Remove script/style/noscript/template contents
                for t in soup(["script", "style", "noscript", "template"]):
                    try:
                        t.decompose()
                    except Exception:
                        pass
                # Remove common hidden elements
            ...
            readable_override = (
                str(kwargs.get("readable_filename", "") or "").strip()
            )
            display_title = readable_override or title
            if not display_title:
                display_title = Path(s3_path).name  # final fallback

            if parse_text_only:
                extracted_text = soup.get_text(separator="\n", strip=True)
                lines = [ln.strip() for ln in extracted_text.splitlines()]
                cleaned = "\n".join([ln for ln in lines if ln])
            else:
                # Backward-compatible minimal parsing
            ...
            success_or_failure = self.split_and_upload(texts, metadata, **kwargs)
        ...
        try:
            ...
            with NamedTemporaryFile(suffix=file_ext) as video_tmpfile:
                ...
                self.s3_client.download_fileobj(
                    Bucket=os.environ["S3_BUCKET_NAME"],
                    Key=s3_path,
                    Fileobj=video_tmpfile,
                )

                # try with original file first
                try:
                    mp4_version = AudioSegment.from_file(
                        video_tmpfile.name, file_ext[1:]
                    )
                except Exception as e:
                    print("Applying moov atom fix and retrying...")
                    # Fix the moov atom issue using FFmpeg
                    fixed_video_tmpfile = NamedTemporaryFile(
                    ...
                    mp4_version = AudioSegment.from_file(
                        fixed_video_tmpfile.name, file_ext[1:]
                    )

            # save the extracted audio as a temporary webm file
            with NamedTemporaryFile(
                suffix=".webm", dir=media_dir, delete=False
            ) as webm_tmpfile:
            ...
            err = (
                f"❌❌ Error in (VIDEO ingest): `{inspect.currentframe().f_code.co_name}`: {e}\nTraceback:\n",
                traceback.format_exc(),
            )
            print(err)
            sentry_sdk.capture_exception(e)
    ...
    def _ingest_single_docx(self, s3_path: str, course_name: str, **kwargs) -> str:
        try:
            with NamedTemporaryFile() as tmpfile:
                self.s3_client.download_fileobj(
                    Bucket=os.getenv("S3_BUCKET_NAME"), Key=s3_path, Fileobj=tmpfile
                )
            ...
            err = (
                f"❌❌ Error in (DOCX ingest): `{inspect.currentframe().f_code.co_name}`: {e}\nTraceback:\n",
                traceback.format_exc(),
            )
            print(err)
            sentry_sdk.capture_exception(e)
    ...
    def _ingest_single_srt(self, s3_path: str, course_name: str, **kwargs) -> str:
        try:
            ...
            err = (
                f"❌❌ Error in (SRT ingest): `{inspect.currentframe().f_code.co_name}`: {e}\nTraceback:\n",
                traceback.format_exc(),
            )
            print(err)
            sentry_sdk.capture_exception(e)
    ...
    def _ingest_single_excel(self, s3_path: str, course_name: str, **kwargs) -> str:
        try:
            ...
            err = (
                f"❌❌ Error in (Excel/xlsx ingest): `{inspect.currentframe().f_code.co_name}`: {e}\nTraceback:\n",
                traceback.format_exc(),
            )
            print(err)
            sentry_sdk.capture_exception(e)
    ...
    def _ingest_single_image(self, s3_path: str, course_name: str, **kwargs) -> str:
        ...
        """
            ...
            err = (
                f"❌❌ Error in (png/jpg ingest): `{inspect.currentframe().f_code.co_name}`: {e}\nTraceback:\n",
                traceback.format_exc(),
            )
            print(err)
            sentry_sdk.capture_exception(e)
    ...
    def _ingest_single_csv(self, s3_path: str, course_name: str, **kwargs) -> str:
        try:
            ...
            err = (
                f"❌❌ Error in (CSV ingest): `{inspect.currentframe().f_code.co_name}`: {e}\nTraceback:\n",
                traceback.format_exc(),
            )
            print(err)
            sentry_sdk.capture_exception(e)
    ...
    def _ingest_single_pdf(self, s3_path: str, course_name: str, **kwargs):
        ...
        try:
            with NamedTemporaryFile() as pdf_tmpfile:
                # download from S3 into pdf_tmpfile
                self.s3_client.download_fileobj(
                    Bucket=os.getenv("S3_BUCKET_NAME"), Key=s3_path, Fileobj=pdf_tmpfile
                )
                ### READ OCR of PDF
                try:
                        ...
                        with NamedTemporaryFile(suffix=".png") as first_page_png:
                            ...
                            with open(first_page_png.name, "rb") as f:
                                ...
                                self.s3_client.upload_fileobj(
                                    f, os.getenv("S3_BUCKET_NAME"), s3_upload_path
                                )
                ...
                if has_words:
                    success_or_failure = self.split_and_upload(
                        texts=pdf_texts, metadatas=metadatas, **kwargs
                    )
                else:
                    print("⚠️ PDF IS EMPTY -- OCR-ing the PDF.")
                    success_or_failure = self._ocr_pdf(
                        s3_path=s3_path, course_name=course_name, **kwargs
                    )

                return success_or_failure
        except Exception as e:
            err = (
                f"❌❌ Error in PDF ingest (no OCR): `{inspect.currentframe().f_code.co_name}`: {e}\nTraceback:\n",
                traceback.format_exc(),
            )  # type: ignore
            print(err)
            sentry_sdk.capture_exception(e)
    ...
    def _ocr_pdf(self, s3_path: str, course_name: str, **kwargs):
        ...
        try:
            ...
            err = (
                f"❌❌ Error in PDF ingest (with OCR): `{inspect.currentframe().f_code.co_name}`: {e}\nTraceback:\n",
                traceback.format_exc(),
            )
            print(err)
            sentry_sdk.capture_exception(e)
    ...
    def _ingest_single_txt(self, s3_path: str, course_name: str, **kwargs) -> str:
        ...
        try:
            ...
            success_or_failure = self.split_and_upload(
                texts=text, metadatas=metadatas, **kwargs
            )
            return success_or_failure
        except Exception as e:
            err = (
                f"❌❌ Error in (TXT ingest): `{inspect.currentframe().f_code.co_name}`: {e}\nTraceback:\n",
                traceback.format_exc(),
            )
            print(err)
            sentry_sdk.capture_exception(e)
    ...
    def _ingest_single_ppt(self, s3_path: str, course_name: str, **kwargs) -> str:
        ...
        try:
            ...
            err = (
                f"❌❌ Error in (PPTX ingest): `{inspect.currentframe().f_code.co_name}`: {e}\nTraceback:\n",
                traceback.format_exc(),
            )
            print(err)
            sentry_sdk.capture_exception(e)
    ...
    def ingest_github(self, github_url: str, course_name: str) -> str:
        ...
        try:
            ...
            repo = Repo.clone_from(
                github_url, to_path=repo_path, depth=1, clone_submodules=False
            )
    ...
    def split_and_upload(
        self, texts: List[str], metadatas: List[Dict[str, Any]], **kwargs
    ):
        ...
        try:
            ...
            contexts: List[Document] = text_splitter.create_documents(
                texts=texts, metadatas=metadatas
            )
            input_texts = [
                {"input": context.page_content, "model": "text-embedding-ada-002"}
            ...
            if is_duplicate:
                self.posthog.capture(
                    ...
                    properties={
                        "course_name": metadatas[0].get("course_name", None),
                        "s3_path": metadatas[0].get("s3_path", None),
                        "readable_filename": metadatas[0].get(
                            "readable_filename", None
                        ),
                        "url": metadatas[0].get("url", None),
                        "base_url": metadatas[0].get("base_url", None),
                        "is_duplicate": True,
            ...
            print(
                f"⏰ embeddings runtime: {(time.monotonic() - embeddings_start_time):.2f} seconds"
            )
            # parse results into dict of shape page_content -> embedding
            embeddings_dict: dict[str, List[float]] = {
                item[0]["input"]: item[1]["data"][0]["embedding"]
            ...
            response = (
                self.supabase_client.table(
                    os.getenv("REFACTORED_MATERIALS_SUPABASE_TABLE")
                )
                .insert(document)
                .execute()
            )  # type: ignore
                ...
                if groups:
                    ...
                    if contexts[0].metadata.get("url"):
                        data, count = self.supabase_client.rpc(
                            ...
                            {
                                "p_course_name": contexts[0].metadata.get(
                                    "course_name"
                                ),
                                "p_s3_path": contexts[0].metadata.get("s3_path"),
                                "p_url": contexts[0].metadata.get("url"),
                                "p_readable_filename": contexts[0].metadata.get(
                                    "readable_filename"
                                ),
                                "p_doc_groups": groups,
                            },
    ...
    def check_for_duplicates(
        self, texts: List[Dict], metadatas: List[Dict[str, Any]]
    ) -> bool:
        """
        ...
        if incoming_s3_path:
            ...
            supabase_contents = (
                self.supabase_client.table(doc_table)
                .select("id", "contexts", "s3_path")
                .eq("course_name", course_name)
                .like("s3_path", "%" + original_filename + "%")
                .order("id", desc=True)
                .execute()
            )
            supabase_contents = supabase_contents.data
            print(
                f"No. of S3 path based records retrieved: {len(supabase_contents)}"
        ...
        if len(supabase_contents) > 0:  # a doc with same filename exists in Supabase
            for record in supabase_contents:
                ...
                print(
                    "Original filename: ",
                    original_filename,
                    "Current SQL filename: ",
                    sql_filename,
                )

                if (
                    original_filename == sql_filename
                ):  # compare og s3_path/url with incoming s3_path/url
            ...
            if exact_doc_exists:
                ...
                if (
                    supabase_whole_text == current_whole_text
                ):  # matches the previous file
                    print(f"Duplicate ingested! 📄 s3_path/url: {original_filename}.")
                    return True

                else:  # the file is updated
                    print(
                        f"Updated file detected! Same filename, new contents. 📄s3_path/url: {original_filename}"
                    )
    ...
    def delete_data(self, course_name: str, s3_path: str, source_url: str):
        ...
        print(
            f"Deleting {s3_path} from S3, Qdrant, and Supabase for course {course_name}"
        )
        ...
        try:
            ...
            if s3_path:
                ...
                try:
                    self.supabase_client.from_(
                        os.environ["REFACTORED_MATERIALS_SUPABASE_TABLE"]
                    ).delete().eq("s3_path", s3_path).eq(
                        "course_name", course_name
                    ).execute()
                except Exception as e:
